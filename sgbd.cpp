...
public:
    ...
    short int *tamAtributos;
    short int *tipoAtributo;
...
public:
    ...
    short int calcularCapacidadTotal()
    {
...
const short int bytesNombreColumna = 30;
const short int bytesTipoColumna = 10;
const short int bytesTamColumna = 8;
...
void cargarRegistros(string& nombreRelacion)
{
    fstream relacion(nombreRelacion + ".txt", std::ios::app);
    if (!relacion.is_open()) return;
    ...
    while (true)
    {
        std::cout << "\tArchivo de carga: ";
        std::cin >> nombre;
        ifstream archivoCarga(nombre);
        if (archivoCarga.is_open()) break;
        else std::cout << "\tArchivo no encontrado! Ingrese una archivo valido\n";
    }
    
    ifstream archivoCarga(nombre);
    //Ignora la fila de encabezados
    while (archivoCarga >> noskipws >> letra) if (letra == '\n') break;

    bool cadena = false;
    while (archivoCarga >> noskipws >> letra)
    {
        if (letra == ',') break;
    }
